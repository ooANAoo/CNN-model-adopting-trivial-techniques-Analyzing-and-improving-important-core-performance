...
class PoseCNN(nn.Module):
    def __init__(self, num_joints):
        ...
        # 수정된 부분: 240x135 이미지 크기에 맞춰 첫 번째 완전 연결 레이어의 입력 차원 조정
        self.fc1 = nn.Linear(128 * 30 * 16, 512)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 128 * 30 * 16)  # 특성 맵을 1차원으로 변환
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("C:/Users/KimDoHyum/Desktop/folders/자세교정 ai/013.피트니스자세_sample/model10987654321.pth"))
    model.eval()
    print("데이터 불러오기")
    try :
        ...
        # 모델 평가 실행
        mse, mae, rmse, r2 = evaluate_model(model, train_loader)
...
def detect_joints(image_path):
    ...
    image = transform(image).unsqueeze(0).to(device)  # 이미지를 GPU로 이동
    with torch.no_grad():
        outputs = model(image)
...
# 이미지에서 관절 검출 및 시각화
# image_path = "C:/Users/KimDoHyum/Desktop/folders/자세교정 ai/013.피트니스자세_sample/data/realData/1/C/033-1-1-21-Z17_C/033-1-1-21-Z17_C-0000025.jpg"
